...
export const AdminView: React.FC<AdminViewProps> = ({ 
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      // Fix: Added explicit return type : Lead to the map function to ensure 'status' is not inferred as generic string
      const newLeads: Lead[] = data.slice(1).map((row, index): Lead => ({
        id: `temp-${Date.now()}-${index}`,
        name: String(row[0] || 'Sem Nome'),
        contest: String(row[1] || 'Geral'),
        phone: String(row[2] || ''),
        status: 'PENDING',
      ...
      if (newLeads.length > 0) {
        ...
        alert(`${newLeads.length} leads preparados para importação!`);
      } else {
        alert("Nenhum lead válido encontrado. Verifique se a Coluna 3 contém os telefones.");
      }
    };
      ...
      {activeTab === 'users' && (
        <div className="bg-white rounded-[2.5rem] border-2 border-gray-100 overflow-hidden shadow-sm">
          <div className="p-6 bg-gray-50/50 border-b flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <button onClick={onDistributeLeads} className="w-full sm:w-auto bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black text-sm shadow-lg shadow-indigo-100 active:scale-95 transition-all flex items-center justify-center gap-2"><RefreshCw className="w-4" /> DISTRIBUIR LEADS</button>
          </div>
      ...
      {activeTab === 'leads' && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white p-8 rounded-[2.5rem] border-2 border-gray-100 flex flex-col items-center justify-center text-center space-y-4 group cursor-pointer hover:border-indigo-600 transition-all" onClick={() => fileInputRef.current?.click()}>
              ...
              <div><h4 className="font-black text-gray-900">IMPORTAR PLANILHA</h4><p className="text-xs text-gray-400">Arraste ou clique para subir (.xlsx, .csv)</p></div>
            </div>
            <div className="bg-white p-8 rounded-[2.5rem] border-2 border-gray-100 flex flex-col items-center justify-center text-center space-y-4">
               ...
               <div><h4 className="font-black text-gray-900">{leads.length} LEADS NA BASE</h4><p className="text-xs text-gray-400">{leads.filter(l => !l.assignedTo).length} aguardando distribuição</p></div>
            </div>
          </div>

          <div className="bg-white rounded-[2.5rem] border-2 border-gray-100 overflow-hidden shadow-sm">
            <div className="p-6 bg-gray-50/50 border-b flex justify-between items-center"><h3 className="font-black text-lg">LISTA DE LEADS</h3><div className="flex gap-2"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Buscar lead..." className="pl-10 pr-4 py-2 bg-white border rounded-xl text-xs outline-none focus:border-indigo-600 w-40 sm:w-64" /></div></div></div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {leads.slice(0, 50).map(l => (
                    <tr key={l.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-8 py-4"><p className="text-sm font-bold text-gray-900">{l.name}</p><span className="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">{l.contest}</span></td>
                      <td className="px-8 py-4 font-mono text-xs font-bold text-indigo-600">{l.phone}</td>
                      ...
                      <td className="px-8 py-4 text-xs font-bold text-gray-500">{users.find(u => u.id === l.assignedTo)?.nome || '—'}</td>
                    </tr>
                  ))}
